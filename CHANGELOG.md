...
- **저장소 범위를 문서/스킬 전용으로 재정의했습니다.** `gstack-ko`는 이제 한국어 문서와 `SKILL.md`를 제공하는 저장소로 명시되며, 실행 코드와 테스트는 upstream `gstack`의 책임으로 분리됩니다.
- **공개 sync 정책을 추가했습니다.** `docs/translation-sync-policy.md`에 upstream sync 시 무엇을 가져오고 무엇을 제외하는지, 어떤 PR을 받지 않는지, 브라우저 의존 스킬을 어떻게 다룰지 기준을 문서화했습니다.
- **루트 문서를 정리했습니다.** `README.md`, `CONTRIBUTING.md`, `ARCHITECTURE.md`, `CLAUDE.md`, `TODOS.md`를 문서/스킬 전용 저장소 방향에 맞게 다시 썼습니다.
- **범위 밖 자산을 제거했습니다.** 실행 코드, 테스트, 템플릿, 빌드/설치 관련 파일을 저장소에서 제거해 upstream 포크처럼 보이던 구조를 정리했습니다.
...
- **발견한 문제를 직접 수정할 수도 있습니다.** `/qa-design-review`는 동일한 디자이너 시각의 감사를 실행한 후, `style(design):` 커밋과 수정 전후 스크린샷을 통해 디자인 이슈를 반복적으로 수정합니다. 기본적으로 CSS-safe 모드로 동작하며, 스타일 변경에 최적화된 엄격한 자기 규제 휴리스틱을 적용합니다.
...
- `gen-skill-docs.ts`에 `{{DESIGN_METHODOLOGY}}` 리졸버 추가 — `/plan-design-review`와 `/qa-design-review` 템플릿 양쪽에 공유 디자인 감사 방법론을 주입합니다. `{{QA_METHODOLOGY}}` 패턴을 따릅니다.
