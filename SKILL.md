...
version: 1.1.0
description: |
  QA 테스트와 사이트 도그푸딩을 위한 빠른 헤드리스 브라우저입니다. 모든 URL 탐색,
  요소 상호작용, 페이지 상태 검증, 액션 전후 diff, 주석 스크린샷, 반응형 레이아웃,
  폼/업로드 테스트, 다이얼로그 처리, 요소 상태 assertion을 수행합니다.
  명령당 약 100ms이며, 기능 테스트/배포 검증/사용자 플로우 도그푸딩/증거 기반 버그 리포트가 필요할 때 사용하세요.
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
